...
func runSnapshot(ctx context.Context, token, owner, repo string, prNumber int, enableLinks bool) int {
	...
	// Print each check (avg shows "--" in snapshot mode since historical data isn't fetched)
	exitCode := 0
	for _, check := range checkRuns {
		...
		avgText := tui.FormatAverage(check, nil)
...
func run(args []string) int {
	...
	// Create model
	model := tui.NewModel(ctx, token, owner, repo, prNumber, cfg.RefreshInterval, styles, cfg.EnableLinks, cfg.HistoricalSampleSize)
